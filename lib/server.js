...
module.exports = function(program)
{
  ...
  if (configData.mb)
  {
    mb.connect(configData.mb, function(error)
    {
      if (error)
      {
        console.log("Failed to connect to message bus, error: " + error);
        return;
      }
        ...
        helpers.writeUploadedFile(configData.repositoriesPath, repositoryInfo, request, configData.enableRedeploy || !repositoryInfo.isArtifact, function(error, filename)
        {
          ...
          if (configData.mb)
          {
            helpers.constructMessage(repositoryInfo, request.query, function(error, message)
            {
              if (error)
              {
                console.log("Failed to construct message, error: " + error);
                return;
              }
              ...
              if (message)
              {
                mb.publish(configData.mb.routingKey, message, function(error)
                {
                  if (error)
                  {
                    console.log("Failed to publish message, error: " + error);
                    return;
                  }
                 
                  console.log("Published message to the message bus, ", JSON.stringify(message));
                });
              }
            });
          }
