...
exports.decodeRepositoryUrl = function(repository, url, callback)
{
  ...
  var info = {};
  var parts = url.split("/");
  var versionScheme = typeof configData.versionScheme === "undefined" ? "default" : configData.versionScheme;
  var versionSettings = {};
  var enableRedeploy = typeof configData.enableRedeploy === "undefined" ? false : configData.enableRedeploy;
  
  if (configData.repos && configData.repos[repository])
  {
    if (typeof configData.repos[repository].enableRedeploy !== "undefined")
    {
      enableRedeploy = configData.repos[repository].enableRedeploy;    
    }
    
    if (typeof configData.repos[repository].versionScheme !== "undefined")
    {
      versionScheme = configData.repos[repository].versionScheme;    
    }
  }
...
exports.getVersions = function(repositoryPath, repositoryInfo)
{
  ...
  {
    console.error("Failed to sort versions!");
    console.log(JSON.stringify(repositoryInfo));
    console.error(e);
    return false;
  }
...
exports.updateArtifactMetadata = function(repositoryPath, repositoryInfo, callback)
{
  ...
  if (!versionInfo)
  {
    callback("Failed to get version information!");
    return;
  }
  ...
  ensureDir(artifactPath, 0755, function(error)
  {
    exports.ensureArtifactMetadata(repositoryPath, repositoryInfo, function(error)
    {
      ...
      else if (repositoryInfo.filename === "reserve")
      {
        ...
        if (!versionInfo)
        {
          callback("Failed to get version information while reserving!");
          return;
        }
