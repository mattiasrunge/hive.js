...
exports = module.exports = function directory(root, options){
  ...
  return function directory(req, res, next) {
    ...
    if (~path.indexOf('\0')) return next(exports.error(400));

    // malicious path, forbidden
    if (0 != path.indexOf(root)) return next(exports.error(403));
    ...
    fs.stat(path, function(err, stat){
      ...
      fs.readdir(path, function(err, files){
        ...
        // not acceptable
        if (!type) return next(exports.error(406));
...
exports.html = function(req, res, files, next, dir, showUp, icons){
  fs.readFile(__dirname + '/../resources/public/directory.html', 'utf8', function(err, str){
    if (err) return next(err);
    fs.readFile(__dirname + '/../resources/public/style.css', 'utf8', function(err, style){
      if (err) return next(err);
...
function html(files, dir, useIcons) {
  return '<ul id="files">' + files.map(function(file){
    ...
    return '<li><a href="'
      + exports.normalizeSlashes(normalize(path.join('/')))
      + '" class="'
      + classes.join(' ') + '"'
      + ' title="' + file + '">'
      + icon + file + '</a></li>';
...
function load(icon) {
  ...
  return cache[icon] = fs.readFileSync(__dirname + '/../resources/public/icons/' + icon, 'base64');
}
